<?php 
	...
	$consulta = sprintf("SELECT count(*) FROM tbl_usuarios WHERE Correo = '%s'",$conexion->antiInyeccion($correo));
		...
		if ($resultado->fetch_assoc()['count(*)'] == '0') {		
			...
			$consulta=sprintf("INSERT into tbl_estudiantes(usuarioID, SeccionID, institucionID) values('%s','%s','%s')",$conexion->antiInyeccion($usuarioid['usuarioID']),$conexion->antiInyeccion($seccion),$conexion->antiInyeccion($_SESSION['Institucion']));
			$conexion->ejecutarconsulta($consulta);
			...
			$consulta=sprintf("SELECT estudianteID FROM tbl_estudiantes WHERE usuarioID='%s'",$conexion->antiInyeccion($usuarioid['usuarioID']));
			$aux = $conexion->ejecutarconsulta($consulta);
			$estudianteid=$aux->fetch_array();

			$consulta=sprintf("INSERT INTO tbl_estudiantesxcurso(CursoID, estudianteID) VALUES('%s','%s')",$conexion->antiInyeccion($curso),$conexion->antiInyeccion($estudianteid['estudianteID']));
			$conexion->ejecutarconsulta($consulta);
			...
			$var = "Estudiante agregado con exito";		
			echo "<script>
					alert('".$var."');
